...
class mesh_motion_elastic:
  ...
  def __init__(self, WALL_id, Vx, Vy, distance, layers, it, ExistModel=0, nnDir=''):
    ...
    self.loss_WALL_3 = tf.reduce_mean(tf.square(self.X_WALL_3_pred - self.x_WALL_3_tf)) \
                     + tf.reduce_mean(tf.square(self.Y_WALL_3_pred - (self.y_WALL_3_tf + (self.it+1)*0.05*tf.sin(np.pi*self.x_WALL_4_tf))))
    self.loss_WALL_4 = tf.reduce_mean(tf.square(self.X_WALL_4_pred - self.x_WALL_4_tf)) \
                     + tf.reduce_mean(tf.square(self.Y_WALL_4_pred - (self.y_WALL_4_tf - (self.it+1)*0.05*tf.sin(np.pi*self.x_WALL_4_tf))))

    self.loss_boundary = self.loss_WALL_1 + self.loss_WALL_2 + self.loss_WALL_3 + self.loss_WALL_4
    self.loss = self.loss_residual + 25*self.loss_boundary
  ...
  def train(self, iter, batch_size=512):
