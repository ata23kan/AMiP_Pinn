...
class mesh_motion_elastic:
  # Initialize the class
  def __init__(self, WALL_id, line_id, res_id,
                     Vx, Vy, layers, 
                     ExistModel=0, nnDir=''):
    ...
    self.x_LINE = self.Vx[line_id, :]
    self.y_LINE = self.Vy[line_id, :]

    self.res_x = self.Vx[res_id, :]
    self.res_y = self.Vy[res_id, :]
    ...
    self.x_LINE_tf = tf.placeholder(tf.float32, shape=(None, 1))
    self.y_LINE_tf = tf.placeholder(tf.float32, shape=(None, 1))
    ...
    self.X_LINE_pred, self.Y_LINE_pred = self.net_mesh(self.x_LINE_tf, self.y_LINE_tf)

    self.f_X_pred, self.f_Y_pred = self.net_elastic(self.x_c_tf, self.y_c_tf)
    ...
    self.loss_WALL_1 = tf.reduce_mean(tf.square(self.X_WALL_1_pred - self.x_WALL_1_tf)) \
                     + tf.reduce_mean(tf.square(self.Y_WALL_1_pred - self.y_WALL_1_tf))
    self.loss_WALL_2 = tf.reduce_mean(tf.square(self.X_WALL_2_pred - self.x_WALL_2_tf)) \
                     + tf.reduce_mean(tf.square(self.Y_WALL_2_pred - self.y_WALL_2_tf))
    self.loss_WALL_3 = tf.reduce_mean(tf.square(self.X_WALL_3_pred - self.x_WALL_3_tf)) \
                     + tf.reduce_mean(tf.square(self.Y_WALL_3_pred - self.y_WALL_3_tf))
    self.loss_WALL_4 = tf.reduce_mean(tf.square(self.X_WALL_4_pred - self.x_WALL_4_tf)) \
                     + tf.reduce_mean(tf.square(self.Y_WALL_4_pred - self.y_WALL_4_tf))

    xr, yr = rotation(self.x_LINE_tf, self.y_LINE_tf, theta=self.theta, name='rad')                   

    self.loss_LINE = tf.reduce_mean(tf.square(self.X_LINE_pred - xr)) \
                   + tf.reduce_mean(tf.square(self.Y_LINE_pred - yr))

    self.loss_boundary = self.loss_WALL_1 + self.loss_WALL_2 + self.loss_WALL_3 + self.loss_WALL_4
    self.loss_line = self.loss_LINE
    self.loss = self.loss_residual + self.loss_boundary + self.loss_line
  ...
  def net_elastic(self, x, y):
    ...
    # Mesh motion using linear elastic equations
    X_x = tf.gradients(X, x)[0]
    X_y = tf.gradients(X, y)[0]
    Y_x = tf.gradients(Y, x)[0]
    Y_y = tf.gradients(Y, y)[0]

    X_xy = tf.gradients(X_x, y)[0]
    X_yx = tf.gradients(X_y, x)[0]
    Y_xy = tf.gradients(Y_x, y)[0]
    Y_yx = tf.gradients(Y_y, x)[0]
    ...
    # f_u:=Sxx_x+Sxy_y
    f_X = lambda_a*(X_xx + Y_yx) + mu_a*(2*X_xx + X_yy + Y_xy)
    f_Y = mu_a*(X_yx + Y_xx + 2*Y_yy) + lambda_a*(X_xy + Y_yy)
  ...
  def train(self, iter):
    ...
    tf_dict = {self.x_c_tf: self.res_x, self.y_c_tf: self.res_y,
               ...
               self.x_LINE_tf: self.x_LINE, self.y_LINE_tf: self.y_LINE}
    ...
    for it in range(iter):
      ...
      self.loss_hist.append(self.sess.run(self.loss, tf_dict))
      self.loss_f_hist.append(self.sess.run(self.loss_residual, tf_dict))
      self.loss_line_hist.append(self.sess.run(self.loss_line, tf_dict))
      self.loss_bd_hist.append(self.sess.run(self.loss_boundary, tf_dict))
    return self.loss
...
class mesh_motion_elastic_hardBC:
  # Initialize the class
  ...
  def net_elastic(self, x, y):
    ...
    X = self.Xg + self.distance*X_nn
    Y = self.Yg + self.distance*Y_nn

    lambda_a = self.lambda_a
    mu_a = self.mu_a

    # Mesh motion using linear elastic equation
    X_x = tf.gradients(X, x)[0]
    X_y = tf.gradients(X, y)[0]
    Y_x = tf.gradients(Y, x)[0]
    Y_y = tf.gradients(Y, y)[0]
