...
from kovasznay_model import DNN, DNN_FF
...
if FF:
	...
	W = torch.normal(mean=0.0, std=1.0, size=(2, width // 2)).float()
else:
	model = DNN()
...
if FF:
	ruv = model.calc(torch.Tensor(X), torch.Tensor(Y), W).detach().numpy()
else:
...
u_exact, v_exact = exact(torch.Tensor(X), torch.Tensor(Y))
...
# fileName = save_folder + 'kovasznay_exact.vtu'
...
# fileName = save_folder + 'kovasznay_error.vtu'
# bns_plot_fields(fileName, mesh_file,
#                 field_rho, field_u, field_v)
# plt.figure()
